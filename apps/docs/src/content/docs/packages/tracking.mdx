...
```typescript
import { buildGA4Script, buildPlausibleScript, buildFathomScript } from '@ccbd-seo/tracking';
...
```typescript
import { createGA4Client, createPlausibleClient } from '@ccbd-seo/tracking';
...
| Import | Description |
|---|---|
| `@ccbd-seo/tracking` | Script builders, consent manager, API clients |
| `@ccbd-seo/tracking/react` | `AnalyticsScript` and `ConsentBanner` components |
