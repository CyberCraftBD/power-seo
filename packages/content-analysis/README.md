...
The linguistic heuristics in this package — transition word detection, experience and E-E-A-T phrase detection (e.g. "I tested", "editor's note"), search intent modifiers, and syllable-based readability counting — are built on hardcoded **English** patterns and word lists.

- **Language-agnostic checks** — heading structure, internal/external links, media counts, word counts, and schema/date metadata work reliably for content in any language
- **English-only checks** — transition words, E-E-A-T phrase detection, intent classification, and readability scoring produce unreliable results for non-English content

Non-English content will receive dependable structural scores, but the linguistic scores should not be trusted. If you analyze non-English content, consider disabling the linguistic checks via `disabledChecks`.
